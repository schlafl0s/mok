...
export default function License() {
  ...
  useEffect(() => {
    ...
    const fetchPageData = async () => {
      try {
        const response = await fetch(`https://wp.doctor-mok.ru/wp-json/wp/v2/pages/549`);
  ...
  useEffect(() => {
    ...
    if (mediaId !== null) {
      const fetchPdfUrl = async () => {
        try {
          const response = await fetch(`https://wp.doctor-mok.ru/wp-json/wp/v2/media/${mediaId}`);
          const data = await response.json();
  ...
  useEffect(() => {
    ...
    if (legalMediaId !== null) {
      const fetchLegalPdfUrl = async () => {
        try {
          const response = await fetch(`https://wp.doctor-mok.ru/wp-json/wp/v2/media/${legalMediaId}`);
          const data = await response.json();
